...
worksheet.write('C1','Domain Expiration:')
count = 2
results_count = 0
domainExp = match1 = match2 = matchString = resultstring = ''
regex1 = r"(?<=expiration_date\"\:.\[)(.*?)(?=(\]))"
regex2 = r"(?<=expiration_date\"\:.)(.*?)(?=(\,))"

for dom in domains:
    worksheet.write('A'+ str(count), dom)
    resultstring = str(results[results_count])
    worksheet.write('B'+ str(count), resultstring)
    if "no match for" in resultstring:
        worksheet.write('C'+ str(count), 'null')
        count = count + 1
        results_count = results_count + 1
        continue
    match1 = re.search(regex1, resultstring, re.DOTALL)
    if match1:
        matchString = ("{match}".format(match = match1.group()))
        #print('match1',match1)
        domainExp = str(matchString[1:])
        worksheet.write('C'+ str(count), domainExp)
        match1 = ''
    else:
        match2 = re.search(regex2, resultstring, re.DOTALL)
        matchString = ("{match}".format(match = match2.group()))
        domainExp = str(matchString)
        worksheet.write('C'+ str(count), domainExp)
        match2 = ''
    count = count + 1
    results_count = results_count + 1
